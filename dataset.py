...
def create_dataset(num_currencies=500, num_transactions=100000, insert_cycle=True):
    ...
    Returns:
        ...
        num_currencies: Number of currency nodes
    """
    # Generate synthetic exchange rates and weights
    edges = []
    ...
    # (Optional) Insert a known negative cycle for demonstration
    if insert_cycle:
        # Ensure cycle nodes are within range by using modulo
        ...
        random.shuffle(node_positions)  # Shuffle to make it more random
        
        # Nodes in the cycle
        cycle_nodes = [node_positions[i % len(node_positions)] for i in range(5)]
        cycle_nodes[4] = cycle_nodes[0]  # Close the cycle
        
        rates = [1.05, 0.98, 1.02, 1.03]  # choose rates whose product >1
        ...
        for i in range(len(cycle_nodes)-1):
            u = cycle_nodes[i]
            v = cycle_nodes[i+1]
            # Skip self-loops if we have too few currencies
            if u == v:
                continue
            w = -math.log(rates[i])
            edges.append((u, v, w))

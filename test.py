...
def run_test_suite():
    ...
    with Progress(
        ...
        for currencies, transactions, insert_cycle in test_cases:
            test_desc = f"Testing with {currencies} currencies, {transactions} transactions, " + \
...
def generate_report(results, success_count, failure_count):
    ...
    console.print(f"Success rate: [{'green' if success_count == total_tests else 'yellow'}]{success_count/total_tests*100:.1f}%[/]")
    ...
    if failure_count > 0:
        ...
        for i, result in enumerate(results):
            if not result["success"]:
                console.print(f"\n[red]Failure {i+1}:[/red]")
                console.print(f"Parameters: {result['currencies']} currencies, {result['transactions']} transactions, insert_cycle={result['insert_cycle']}")
    ...
    console.print(f"Tests with inserted cycle that found arbitrage: {arbitrage_counts['with_cycle']}/{len([r for r in results if r['insert_cycle']])}")
    console.print(f"Tests without inserted cycle that found arbitrage: {arbitrage_counts['without_cycle']}/{len([r for r in results if not r['insert_cycle']])}")

def main():
    console.print("[bold]Currency Arbitrage Detection Test Suite[/bold]")
    console.print("This test suite will run the algorithm with various parameter combinations.")

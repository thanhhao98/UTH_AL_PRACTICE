...
def fetch_ecb_daily_rates():
    ...
    try:
        ...
        df = pd.read_xml(
            response.content, xpath=".//eurofxref:Cube[@time]", namespaces=namespaces
        )

        date_str = df.iloc[0]["time"] if not df.empty else "latest"
        console.print(f"Using exchange rates from {date_str}", style="green")

        rates_df = pd.read_xml(
            response.content,
            xpath=".//eurofxref:Cube[@currency]",
...
def fetch_ecb_historical_rates(days=90):
    ...
    try:
        console.print(
            f"Fetching historical exchange rates (past {days} days) from ECB...",
            style="cyan",
        )
...
def create_real_dataset(use_historical=False, base_currency="EUR", num_currencies=None):
    ...
    if rates_df is None or len(rates_df) == 0:
        console.print(
            "Failed to retrieve exchange rate data. Using a small sample dataset.",
            style="bold yellow",
        )
    ...
    console.print(
        f"Using {num_currencies_actual} currencies: {', '.join(currency_codes[:min(10, num_currencies_actual)])}{'...' if num_currencies_actual > 10 else ''}",
        style="cyan",
    )
    ...
    for date, group in daily_groups:
        ...
        for i, row1 in day_rates.iterrows():
            ...
            for j, row2 in day_rates.iterrows():
                ...
                edges.append(
                    (currency_to_index[curr1], currency_to_index[curr2], weight)
                )

    console.print(
        f"Created {len(edges)} edges from real exchange rate data", style="green"
    )
...
def create_currency_results_table(
...
if __name__ == "__main__":
    ...
    print(
        f"Currency codes: {currency_codes[:10]}{'...' if len(currency_codes) > 10 else ''}"
    )
